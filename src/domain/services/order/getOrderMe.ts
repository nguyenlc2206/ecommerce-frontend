...
export interface GetOrderMeService<Entity> {
...
@Service()
export class GetOrderMeServiceImpl<Entity extends OrderModel> implements GetOrderMeService<Entity> {
    ...
    async execute(): Promise<Either<OrderModel[], AppError>> {
        ...
        const res = await this.orderApi.getOrderMe();
        if (res?.EC !== 200) {
            /** open snackbar alert */
            dispatch(
                openSnackbar({
