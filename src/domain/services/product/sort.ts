...
import { getListProductSort } from '@ecommerce-frontend/src/infras/data/store/reducers/product';
import { setLoading } from '@ecommerce-frontend/src/infras/data/store/reducers/page';

/** define sort services */
export interface SortProductService<Entity> {
    execute(): Promise<Either<ProductModel[], AppError>>;
}

// ==============================|| SORT PRODUCT SERVICE IMPLEMENT ||============================== //

@Service()
export class SortProductServiceImpl<Entity extends ProductModel> implements SortProductService<Entity> {
    /** init api */
    ...
    async execute(): Promise<Either<ProductModel[], AppError>> {
        ...
        const res = await this.productApi.sort();
        if (res?.EC !== 200) {
            /** open snackbar alert */
            dispatch(
                openSnackbar({
        ...
        /** save data to redux */
        dispatch(getListProductSort(result));
        dispatch(setLoading(false));
