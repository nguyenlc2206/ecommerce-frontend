...
// ==============================|| ACTIVE SERVICE IMPLEMENT ||============================== //

export class ActiveCouponServiceImpl<Entity extends CouponModel> implements ActiveCouponService<Entity> {
    /** init api */
    ...
    async execute(id: string): Promise<Either<string, AppError>> {
        ...
        const res = await this.couponApi.active(id);
        if (res?.EC !== 200) {
            /** open snackbar alert */
            dispatch(
                openSnackbar({
