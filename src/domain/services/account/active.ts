...
// ==============================|| ACTIVE ACCOUNT SERVICE IMPLEMENT ||============================== //

export class ActiveAccountServiceImpl<Entity extends AccountModel> implements ActiveAccountService<Entity> {
    /** init api */
    ...
    async execute(id: string): Promise<Either<string, AppError>> {
        ...
        const res = await this.accountApi.activeAccount(id);
        if (res?.EC !== 200) {
            /** open snackbar alert */
            dispatch(
                openSnackbar({
        ...
        // update redux data user select
        dispatch(activeUser({ ...store.getState().user.userSelect, isDeleted: false, deletedAt: null }));
        return success('okie');
    }
}
