...
// * import hooks
import { useEvent } from '@ecommerce-frontend/src/common/hooks/useEvent';

// * import types
import { MuiOtpInputProps } from '@ecommerce-frontend/src/common/types/otp';
import {
    getFilledArray,
    joinArrayStrings,
    mergeArrayStringFromIndex,
    updateIndex
} from '@ecommerce-frontend/src/application/widgets/otp/helpers/array';
import { split } from '@ecommerce-frontend/src/application/widgets/otp/helpers/string';
import { KEYBOARD_KEY } from '@ecommerce-frontend/src/application/widgets/otp/helpers/event';
...
const MuiOTPInput = React.forwardRef((props: MuiOtpInputProps, propRef: MuiOtpInputProps['ref']) => {
    const {
        value = '',
        length = 4,
        isNumber = false,
    ...
    const handleOneInputPaste = (event: React.ClipboardEvent<HTMLInputElement>) => {
        ...
        const characters = mergeArrayStringFromIndex(currentCharacter, split(content), currentInputIndex).map(
            (character, index) => {
